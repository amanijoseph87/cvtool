...
const Input = ({
  ...
  const inputClassName = `block w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700/75 p-2 text-sm text-gray-800 dark:text-gray-100 shadow-md shadow-gray-200 dark:shadow-gray-800 outline-none focus:border-2 focus:border-primary-500 focus:bg-gray-50 dark:focus:bg-gray-700 md:text-base 2xl:p-2.5 transition-colors duration-200`;
    ...
    if (type === "textarea" && props.multipoints) {
      ...
      return (
        <ContentEditable
          ...
          className={twMerge(
            inputClassName,
            "min-h-56  text-sm md:min-h-40 md:text-sm "
          )}
          onChange={(e) => {
  ...
  return (
    <div className={`${span ? "md:col-span-2" : ""}`}>
      {label && (
        <label
          htmlFor={name}
          className="mb-0.5 block text-xs text-gray-700 dark:text-gray-300 md:text-sm 2xl:text-base transition-colors duration-200"
        >
